...
class TestEvents:
    ...
    def test_get_events_wrong_pagination(self, limit, page, status_code, error_type, test_client):
    ...
    def test_get_events_correct_pagination(self, limit, page, events_count, test_client, test_data):
        resp = test_client.get(f"/events/?limit={limit}&page={page}")
    ...
    def test_get_events_by_sensor_id(self, sensor_id, test_client, test_data):
        ...
        expected_events = [event for event in test_data if event["sensor_id"] == sensor_id]
        expected_len = len(expected_events)
        real_len = len(resp.json().get("events"))

        assert resp.status_code == 200 
        assert expected_len == real_len, f"ожидали {expected_len}, а получили {real_len}"
    ...
    def test_filter_events_by_temperature(self, min_temp, max_temp, test_client, test_data):
        ...
        expected = [instance for instance in test_data if min_temp <= instance.get("temperature", -500) <= max_temp]

        assert resp.status_code == 200, resp.json()
        assert len(real) == len(expected), f"ожидали {[x.get('temperature') for x in expected]}, получили {[x.get('temperature') for x in real]}"
    ...
    def test_filter_events_by_humidity(self, min_hum, max_hum, test_client, test_data):
        ...
        expected = [instance for instance in test_data if min_hum <= instance.get("humidity", -1) <= max_hum]

        assert resp.status_code == 200, resp.json()
        assert len(real) == len(expected), f"ожидали {[x.get('humidity') for x in expected]}, получили {[x.get('humidity') for x in real]}"
    ...
    def test_post_incorrect_event(self, temperature, humidity, test_client, db_session):
        ...
        incorrect_events = db_session.query(Event).filter(
            and_(
    ...
    def test_put_event(self, test_client, db_session, test_event):
        ...
        assert resp.status_code == 200, resp.json()
        assert changed_event.temperature == data["temperature"], (changed_event, test_event)
